...
log_response=read_csv('log_responses_for_R.csv')
```
...
covariates=covariates[,-c(1:8)] # remove controls
covariates=as.matrix(cbind(1, covariates))
response=response[, -1]
response=t(response)
log_response=log_response[, -1]
log_response=t(log_response)
```
...
log_olsmod = fosr(Y=log_response[complete.cases(covariates[,rows]),], X = covariates[complete.cases(covariates[,rows]),rows], method='OLS', lambda=0.000001)
...
log_coef_estimate=as.data.frame(log_olsmod$est.func)
log_se_estimate=as.data.frame(log_olsmod$se.func)

write_csv(log_coef_estimate, 'log_funcoef_estimate.csv')
write_csv(log_se_estimate, 'log_funse_estimate.csv')
```
